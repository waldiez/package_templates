...
class PreBuildHook(BuildHookInterface):  # type: ignore
    ...
    def _needs_frontend_build(self) -> bool:
        ...
        last_build_txt = Path(self.root) / ".local" / "last-build.txt"
    ...
    def _build_frontend(self) -> None:
        ...
        dot_local = Path(self.root) / ".local"
        dot_local.mkdir(exist_ok=True, parents=True)
        with open(
            Path(self.root) / ".local" / "last-build.txt",
            "w",
            encoding="utf-8",
        ) as f_write:
            f_write.write(datetime.now(timezone.utc).isoformat())
    ...
    def _find_package_manager(self) -> str:
        ...
        # get from package.json: "packageManager": "bun@1.2.10",
